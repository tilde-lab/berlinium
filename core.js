...
function resp__tags(req, data){
    if (req.tids && req.tids.length){
        ...
        _gui.searchables = data.searchables || [];
...
$(document).ready(function(){
    ...
    var search_field = new autoComplete({selector: '#search_field', minChars: 2, source: function(term, suggest){
        ...
        var suggestions = [];
        for (i=0; i<_gui.searchables.length; i++){
            if (~_gui.searchables[i][1].toLowerCase().indexOf(term)) suggestions.push(_gui.searchables[i]);
        }
        suggest(suggestions);
    },
    renderItem: function (item, search){
        search = search.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
        var re = new RegExp("(" + search.split(' ').join('|') + ")", "gi");
        return '<div class="autocomplete-suggestion" data-tid="' + item[0] + '" data-val="' + item[1] + '">' + item[1].replace(re, "<b>$1</b>") + '</div>';
    },
    onSelect: function(e, term, item){
        var tid = parseInt($(item).attr('data-tid')); // FIXME prevent filling in
        __send('tags', {tids: [tid]});
    }});
